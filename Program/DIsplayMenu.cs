...
namespace bank
{
    ...
    class Tests
    {
        private string dummyString = "Bacon ipsum dolor amet pork chop meatball picanha, venison porchetta leberkas pig brisket rump. Turkey tongue pastrami spare ribs prosciutto sirloin tri-tip jerky doner sausage shoulder chislic burgdoggen t-bone ground round. Shankle kevin hamburger pork belly ball tip tri-tip fatback porchetta beef sirloin landjaeger alcatra. Pork belly corned beef tenderloin frankfurter chuck, doner chicken turducken ball tip buffalo prosciutto shank brisket."; // Used to test rendering of long strings
        private string dummyintString = "Contrarytopopularbelief,LoremIpsumisnotsimplyrandomtext.IthasrootsinapieceofclassicalLatinliteraturefrom45BC,makingitover2000yearsold.RichardMcClintock,aLatinprofessoratHampden-SydneyCollegeinVirginia,lookeduponeofthemoreobscureLatinwords,consectetur,fromaLoremIpsumpassage,andgoingthroughthecitesofthewordinclassicalliterature,discoveredtheundoubtablesource.LoremIpsumcomesfromsections1.10.32and1.10.33of"; // Used to test for breaking interfaces with long words
        public Tests() // Alter as needed to test individual functions.
        {
            testSplitString();
        }
        private void testPhoneValidation()
        {
            int phoneNumber1 = 0212345678;
            int phoneNumber2 = 1234;
            long phoneNumber3 = 1987281241240397;
            int phoneNumber4 = 12;
            string key = "phone";
            Console.WriteLine("Testing phone number validation...");
            printValidatorOutput(key, phoneNumber1.ToString());
            printValidatorOutput(key, phoneNumber2.ToString());
            printValidatorOutput(key, phoneNumber3.ToString());
            printValidatorOutput(key, phoneNumber4.ToString());
        }
        private void testEmailValidation()
        {
            string alpha = "test.test@example.com.au";
            string beta = "test@example@example";
            string gamma = "test@gmail.com";
            string key = "email";
            Console.WriteLine("Testing email validation...");
            printValidatorOutput(key, alpha.ToString());
            printValidatorOutput(key, beta.ToString());
            printValidatorOutput(key, gamma.ToString());
        }
        private void testAccountNumberValidation()
        {
            int alpha = 1231270987;
            string beta = "Sweet sizzling bacon";
            int gamma = 1231;
            string key = "accNo";
            Console.WriteLine("Testing account number validation...");
            printValidatorOutput(key, alpha.ToString());
            printValidatorOutput(key, beta.ToString());
            printValidatorOutput(key, gamma.ToString());
        }
        // Validator regex test
        private void printValidatorOutput(string key, string input)
        {
            Validator v = new Validator();
            string printString = "Result of validating {0} is {1}";
            Console.WriteLine(printString, input, v.validate(key, input));
        }
        // Test bank search function
        private void testBankSearch()
        {
            Bank b = new Bank(true);
            User u = new User("guest", "1234", b);
            b.viewSearch(u);
        }
        private void testInterfaces()
        {
            ...
            Console.ReadKey();
            d.interfaceHeader("welcome to simple banking system");
            d.interfaceBankLogin();
            d.interfaceMessage("Header", "Say hi!");
            d.interfaceModalYesNo("Does this work?", 2);
            d.interfaceMessage("message", dummyString);
            d.interfaceMessage("message", dummyintString);
        }
        private void testMessageInterfaces()
        {
            DisplayMenu d = new DisplayMenu();
            d.interfaceMessage("message", dummyString);
            d.interfaceMessage("message", dummyintString);
        }
        private void testBankInterfaces()
        {
            Bank b = new Bank(true);
            User u = new User("guest", "1234", b);
            b.viewMainMenu(u);
        ...
        private void testFiles()
        {
            ...
            string[] a = fe.getMatchingFiles("accNo");
            foreach (string b in a)
            {
                Console.WriteLine(b);
            }
        }
        // Error log writing test
        private void testErrorLog()
        {
            FileExplorer fe = new FileExplorer();
            Exception e = new Exception();
            fe.log(e.ToString(), "Testing only.");
        }
        // Testing Activity initialisation and writing
        private void testActivity()
        {
            ...
            FileExplorer fe = new FileExplorer();
            Validator v = new Validator();
            Account account = new Account(100001, fe, v);
            foreach (Activity a in account.Activities)
            {
                Console.WriteLine(a.FileOutput());
            }
        }
        private void testSplitString()
        {
            string log = "15.12.2019|Withdraw|200|1000";
